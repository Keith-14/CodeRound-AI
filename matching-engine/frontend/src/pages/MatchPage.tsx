...
interface MatchDetailProps {
  match: MatchResult;
  candidateName: string; // we aren't fetching cand directly except in list if we expand it, but match API needs to return it, Wait, match schema might not return name directly. We need to fetch cand or proxy it.
}
...
export function MatchPage() {
  ...
  useEffect(() => {
    // Load jobs initially
    api.get("/jobs").then(res => setJobs(res.data)).catch(err => console.error("Could not fetch jobs"));
